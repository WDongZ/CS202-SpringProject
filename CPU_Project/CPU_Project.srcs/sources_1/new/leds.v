...
module leds (
    ...
    input	[31:0]	ledaddr,	// 2'b00 means updata the low 16bits of ledout, 2'b10 means updata the high 8 bits of ledout
    ...
    always @ (negedge led_clk or negedge ledrst) begin
		...
		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			if (ledaddr == 32'hfffffc60)
				ledout[15:8] <= ledwdata[7:0];
			else if (ledaddr == 32'hfffffc62 )
